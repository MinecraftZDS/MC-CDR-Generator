...
class CanBreakWidget(ttk.Frame):
    def __init__(self, parent, **kwargs):
        ...
        tk.Label(left, text=bc.get_lang_text("can_break.list_label")).grid(row=0, column=0, sticky='w')
        ...
        self.add_btn = ttk.Button(btn_frame, text=bc.get_lang_text("can_break.add"), command=self._add_new)
        ...
        tk.Label(right, text=bc.get_lang_text("can_break.block_id")).grid(row=0, column=0, sticky='w', padx=5, pady=2)
        self.id_entry = tk.Entry(right, width=40)
        self.id_entry.grid(row=0, column=1, sticky='ew', padx=5, pady=2)

        self.save_btn = ttk.Button(right, text=bc.get_lang_text("can_break.save"), command=self._save_current)
    ...
    def _save_current(self):
        ...
        if not block_id:
            messagebox.showerror(bc.get_lang_text("error.title"), bc.get_lang_text("can_break.error.empty"))
            return
        if self.current_index is None:
    ...
    def get_nbt(self):
        ...
        return f'can_break={{blocks:[{blocks_json}]}}'