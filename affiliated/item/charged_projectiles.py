...
class ChargedProjectilesWidget(ttk.Frame):
    def __init__(self, parent, **kwargs):
        super().__init__(parent, **kwargs)
        self.pack(fill='both', expand=True)

        self.items = []          # each: {"id":str, "components":str}
        ...
        left_frame.grid_propagate(False)          # 防止子控件改变宽度
        left_frame.grid_rowconfigure(1, weight=1)
        left_frame.grid_columnconfigure(0, weight=1)

        tk.Label(left_frame, text=bc.get_lang_text("charged_projectiles.list_label")).grid(row=0, column=0, sticky='w')
        ...
        self.add_btn = ttk.Button(btn_frame, text=bc.get_lang_text("charged_projectiles.add"),
        ...
        right_frame.grid_rowconfigure(2, weight=1)   # components 文本框可伸展

        row = 0
        # ID
        tk.Label(right_frame, text=bc.get_lang_text("charged_projectiles.id")).grid(row=row, column=0, sticky='w', padx=5, pady=2)
        self.id_entry = tk.Entry(right_frame, width=30)
        self.id_entry.grid(row=row, column=1, sticky='ew', padx=5, pady=2)
        row += 1

        # Components
        tk.Label(right_frame, text=bc.get_lang_text("charged_projectiles.components")).grid(row=row, column=0, sticky='nw', padx=5, pady=2)
        ...
        # 保存按钮
        self.save_btn = ttk.Button(right_frame, text=bc.get_lang_text("charged_projectiles.save"),
    ...
    def _refresh_listbox(self):
        ...
        for idx, it in enumerate(self.items):
            self.listbox.insert(tk.END, f"{idx+1}: {it['id']}")
    ...
    def _save_current(self):
        try:
            ...
            if not id_val:
                messagebox.showerror(bc.get_lang_text("error.title"),
                                     bc.get_lang_text("charged_projectiles.error.id_empty"))
                return
            comp = self.components_text.get('1.0', 'end-1c').strip()
            entry = {"id": id_val, "components": comp}
            if self.current_index is None:
    ...
    def get_nbt(self):
        ...
        for it in self.items:
            if it['components']:
                parts.append(f'{{id:"{it["id"]}",components:{{{it["components"]}}}}}')
            else:
                parts.append(f'{{id:"{it["id"]}"}}')
        return f'charged_projectiles=[{",".join(parts)}]'