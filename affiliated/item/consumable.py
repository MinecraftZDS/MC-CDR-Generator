...
class ConsumableWidget(ttk.Frame):
    def __init__(self, parent, **kwargs):
        ...
        tk.Label(left_frame, text=bc.get_lang_text("consumable.effects_list")).grid(row=0, column=0, sticky='w')
        ...
        ttk.Button(btn_frame, text=bc.get_lang_text("consumable.add_effect"),
        ...
        row = 0
        # 动画下拉
        tk.Label(right_frame, text=bc.get_lang_text("consumable.animation")).grid(row=row, column=0, sticky='w', padx=5, pady=2)
        self.animation_combo = ttk.Combobox(right_frame, values=[
            "none", "eat", "drink", "block", "bow", "brush", "crossbow", "spear", "trident", "spyglass", "toot_horn", "bundle"
        ], state='readonly', width=28)
        self.animation_combo.grid(row=row, column=1, sticky='ew', padx=5, pady=2)
        self.animation_combo.current(1)
        row += 1

        # 消耗秒数
        tk.Label(right_frame, text=bc.get_lang_text("consumable.consume_seconds")).grid(row=row, column=0, sticky='w', padx=5, pady=2)
        self.seconds_entry = tk.Entry(right_frame, width=10)
        self.seconds_entry.grid(row=row, column=1, sticky='w', padx=5, pady=2)
        row += 1

        # 粒子复选框
        self.particles_var = tk.BooleanVar(value=True)
        tk.Checkbutton(right_frame, text=bc.get_lang_text("consumable.has_particles"),
                       variable=self.particles_var).grid(row=row, column=0, columnspan=2, sticky='w', padx=5, pady=2)
        row += 1

        # 预览区
        self.preview_frame = tk.Frame(right_frame)
        self.preview_frame.grid(row=row, column=0, columnspan=2, sticky='nsew', pady=5)
        right_frame.grid_rowconfigure(row, weight=1)
        row += 1

        # 保存顶层按钮
        self.save_top_btn = ttk.Button(right_frame, text=bc.get_lang_text("consumable.save_top"),
    ...
    def _display_effect(self, effect):
        ...
        btn = ttk.Button(self.preview_frame, text=bc.get_lang_text("consumable.edit_effect"),
    ...
    def _open_effect_editor(self, existing_effect):
        ...
        win.title(bc.get_lang_text("consumable.effect_dialog_title"))
        win.geometry("700x550")
        win.transient(self)
        win.grab_set()

        tk.Label(win, text=bc.get_lang_text("consumable.effect_type")).pack(anchor='w', padx=5, pady=5)
        ...
        def update_ui(*args):
            ...
            if etype == "apply_effects":
                tk.Label(dynamic_frame, text=bc.get_lang_text("consumable.sub_effects_list")).pack(anchor='w')
                ...
                ttk.Button(sub_btn_frame, text=bc.get_lang_text("consumable.add_sub_effect"), command=add_sub).pack(side='left', padx=5)
                def edit_sub():
                    sel = sub_listbox.curselection()
                    if sel:
                        self._edit_sub_effect(sub_effects[sel[0]], sub_effects, refresh_sublist, win)
                ttk.Button(sub_btn_frame, text=bc.get_lang_text("consumable.edit_sub_effect"), command=edit_sub).pack(side='left', padx=5)
                def remove_sub():
                    sel = sub_listbox.curselection()
                    if sel:
                ...
                tk.Label(dynamic_frame, text=bc.get_lang_text("consumable.remove_ids_list")).pack(anchor='w')
                listbox_frame = tk.Frame(dynamic_frame)
                listbox_frame.pack(fill='both', expand=True)
                ...
                def add_id():
                    ...
                    if val and val not in remove_ids:
                        ...
                        refresh_ids()
                ttk.Button(id_btn_frame, text=bc.get_lang_text("consumable.add_effect_id"), command=add_id).pack(side='left', padx=5)
                def remove_id():
                ...
                tk.Label(dynamic_frame, text=bc.get_lang_text("consumable.diameter")).pack(anchor='w')
                ...
                tk.Label(dynamic_frame, text=bc.get_lang_text("consumable.clear_all_hint")).pack(anchor='w')
        ...
        ttk.Button(win, text=bc.get_lang_text("consumable.save_effect"), command=save).pack(pady=10)
    ...
    def _edit_sub_effect(self, sub_effect, sub_list, refresh_callback, parent_win):
        ...
        dlg.title(bc.get_lang_text("consumable.sub_effect_title"))
    ...
    def _save_top(self):
        try:
            anim = self.animation_combo.get().strip()
            sec = float(self.seconds_entry.get().strip() or "0.0")
            if sec < 0:
                raise ValueError
            part = self.particles_var.get()
            messagebox.showinfo(bc.get_lang_text("success.title"), bc.get_lang_text("msgbox.info.saved"))
        except:
            messagebox.showerror(bc.get_lang_text("error.title"), bc.get_lang_text("consumable.error.seconds_negative"))

    # ---------- NBT 生成 ----------
    def get_nbt(self):
        animation = self.animation_combo.get().strip()
        consume_seconds = float(self.seconds_entry.get().strip() or "0.0")
        has_particles = self.particles_var.get()
        top = {
            "animation": animation,
            "consume_seconds": consume_seconds,
            "has_consume_particles": has_particles,
            "on_consume_effects": self.effects
        }
        nbt_str = json.dumps(top, separators=(',', ':'))
        return f'consumable={nbt_str}'