...
class RecipesWidget(ttk.Frame):
    def __init__(self, parent, **kwargs):
        ...
        tk.Label(left_frame, text=bc.get_lang_text("recipes.list_label")).grid(row=0, column=0, sticky='w')
        ...
        ttk.Button(btn_frame, text=bc.get_lang_text("recipes.add"),
        ...
        tk.Label(right_frame, text=bc.get_lang_text("recipes.id_label")).grid(row=0, column=0, sticky='w', padx=5, pady=2)
        self.id_entry = tk.Entry(right_frame, width=40)
        self.id_entry.grid(row=0, column=1, sticky='ew', padx=5, pady=2)

        self.save_btn = ttk.Button(right_frame, text=bc.get_lang_text("recipes.save"),
    ...
    def _save_current(self):
        ...
        if not val:
            messagebox.showerror(bc.get_lang_text("error.title"),
                                 bc.get_lang_text("recipes.error.empty"))
            return
        if self.current_index is None:
